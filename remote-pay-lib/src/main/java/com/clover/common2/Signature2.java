...
public class Signature2 implements Parcelable {
  ...
  public void transform(int newWidth, int newHeight) {
    //ALog.v(this, "width=%d, new width=%d, height=%d, new height=%d", width, newWidth, height, newHeight);

    if (width != -1 && height != -1) {
      float xFactor = (float) newWidth / width;
      float yFactor = (float) newHeight / height;

      //ALog.v(this, "x factor=%f, y factor=%f", xFactor, yFactor);
      ...
      for (Stroke s : strokes) {
        ...
        for (Point p : s.points) {
          ...
          //ALog.v(this, "stroke %d, point %d: x=%d->%d (%f), y=%d->%d (%f), factor=%f", strokeNum, pointNum, p.x, newX, nx, p.y, newY, ny, factor);
